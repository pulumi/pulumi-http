# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins
import copy
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities

__all__ = [
    'GetHttpRetryArgs',
    'GetHttpRetryArgsDict',
]

MYPY = False

if not MYPY:
    class GetHttpRetryArgsDict(TypedDict):
        attempts: NotRequired[builtins.int]
        """
        The number of times the request is to be retried. For example, if 2 is specified, the request will be tried a maximum of 3 times.
        """
        max_delay_ms: NotRequired[builtins.int]
        """
        The maximum delay between retry requests in milliseconds.
        """
        min_delay_ms: NotRequired[builtins.int]
        """
        The minimum delay between retry requests in milliseconds.
        """
elif False:
    GetHttpRetryArgsDict: TypeAlias = Mapping[str, Any]

@pulumi.input_type
class GetHttpRetryArgs:
    def __init__(__self__, *,
                 attempts: Optional[builtins.int] = None,
                 max_delay_ms: Optional[builtins.int] = None,
                 min_delay_ms: Optional[builtins.int] = None):
        """
        :param builtins.int attempts: The number of times the request is to be retried. For example, if 2 is specified, the request will be tried a maximum of 3 times.
        :param builtins.int max_delay_ms: The maximum delay between retry requests in milliseconds.
        :param builtins.int min_delay_ms: The minimum delay between retry requests in milliseconds.
        """
        if attempts is not None:
            pulumi.set(__self__, "attempts", attempts)
        if max_delay_ms is not None:
            pulumi.set(__self__, "max_delay_ms", max_delay_ms)
        if min_delay_ms is not None:
            pulumi.set(__self__, "min_delay_ms", min_delay_ms)

    @property
    @pulumi.getter
    def attempts(self) -> Optional[builtins.int]:
        """
        The number of times the request is to be retried. For example, if 2 is specified, the request will be tried a maximum of 3 times.
        """
        return pulumi.get(self, "attempts")

    @attempts.setter
    def attempts(self, value: Optional[builtins.int]):
        pulumi.set(self, "attempts", value)

    @property
    @pulumi.getter(name="maxDelayMs")
    def max_delay_ms(self) -> Optional[builtins.int]:
        """
        The maximum delay between retry requests in milliseconds.
        """
        return pulumi.get(self, "max_delay_ms")

    @max_delay_ms.setter
    def max_delay_ms(self, value: Optional[builtins.int]):
        pulumi.set(self, "max_delay_ms", value)

    @property
    @pulumi.getter(name="minDelayMs")
    def min_delay_ms(self) -> Optional[builtins.int]:
        """
        The minimum delay between retry requests in milliseconds.
        """
        return pulumi.get(self, "min_delay_ms")

    @min_delay_ms.setter
    def min_delay_ms(self, value: Optional[builtins.int]):
        pulumi.set(self, "min_delay_ms", value)


